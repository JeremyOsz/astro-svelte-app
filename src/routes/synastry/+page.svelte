<script lang="ts">
  ...
  let storedPerson1Data: any = null;
  let storedPerson2Data: any = null;
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 bg-gray-50 rounded-lg">
            <div class="space-y-2">
              <div class="flex items-center gap-2">
                ...
                <span class="font-medium text-gray-900">Person 1 (Inner Wheel)</span>
              </div>
              <div class="text-sm text-gray-600">
                <div class="font-medium">
                  {#if storedPerson1Data?.chart}
                    {storedPerson1Data.chart.name}
                  {:else if storedPerson1Data?.name}
                    {storedPerson1Data.name}
                  {:else if storedPerson1Data?.date && storedPerson1Data?.city}
                    {storedPerson1Data.date} • {storedPerson1Data.city}
                  {:else}
                    Manual Entry
                  {/if}
                </div>
                <div>
                  {#if storedPerson1Data?.chart}
                    {storedPerson1Data.chart.birthData?.date} • {storedPerson1Data.chart.birthData?.place}
                  {:else if storedPerson1Data?.date && storedPerson1Data?.city}
                    {storedPerson1Data.time || 'Time not set'} • {storedPerson1Data.city}
                  {:else}
                    No data entered
                  {/if}
                </div>
            ...
            <div class="space-y-2">
              <div class="flex items-center gap-2">
                <User class="h-4 w-4 text-gray-600" />
                <span class="font-medium text-gray-900">Person 2 (Outer Wheel)</span>
              </div>
              <div class="text-sm text-gray-600">
                <div class="font-medium">
                  {#if storedPerson2Data?.chart}
                    {storedPerson2Data.chart.name}
                  {:else if storedPerson2Data?.name}
                    {storedPerson2Data.name}
                  {:else if storedPerson2Data?.date && storedPerson2Data?.city}
                    {storedPerson2Data.date} • {storedPerson2Data.city}
                  {:else}
                    Manual Entry
                  {/if}
                </div>
                <div>
                  {#if storedPerson2Data?.chart}
                    {storedPerson2Data.chart.birthData?.date} • {storedPerson2Data.chart.birthData?.place}
                  {:else if storedPerson2Data?.date && storedPerson2Data?.city}
                    {storedPerson2Data.time || 'Time not set'} • {storedPerson2Data.city}
                  {:else}
                    No data entered
                  {/if}
                </div>
              ...
              <div class="space-y-4">
                ...
                <div class="space-y-3">
                 ...
                 <div class="space-y-3">
                   ...
                   <div>
                     <Label for="person1-name">Name</Label>
                     <Input
                       id="person1-name"
                       name="person1Name"
                       type="text"
                       bind:value={person1Name}
              ...
              <div class="space-y-4">
                ...
                <div class="space-y-3">
                 ...
                 <div class="space-y-3">
                   ...
                   <div>
                     <Label for="person2-name">Name</Label>
                     <Input
                       id="person2-name"
                       name="person2Name"
                       type="text"
                       bind:value={person2Name}
                       placeholder="Enter name"
                       class="w-full"
                     />
                   </div>
  ...
  {#if isChartReady}
    ...
    <Accordion.Root type="multiple" value={['analysis', 'education']} class="mb-8">
          ...
          <Card.Root>
            ...
            <Card.Content>
              <SynastryInterpretation 
                ...
                person1Name={storedPerson1Data?.name || storedPerson1Data?.chart?.name || 'Person 1'}
                person2Name={storedPerson2Data?.name || storedPerson2Data?.chart?.name || 'Person 2'}
              />
            </Card.Content>
          </Card.Root>
        </Accordion.Content>
      ...
      <Accordion.Item value="education">
        <Accordion.Trigger class="flex items-center justify-between w-full py-4 text-lg font-semibold text-gray-900">
          <span>Understanding Synastry</span>
        </Accordion.Trigger>
        <Accordion.Content>
          <Card.Root>
            <Card.Header>
