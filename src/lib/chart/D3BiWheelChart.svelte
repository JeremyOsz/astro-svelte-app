<script lang="ts">
  ...
  const chartDimensions = derived(chartState, ($chartState) => {
    ...
    debugLog('D3BiWheelChart: Chart dimensions calculated:', dimensions);
    return dimensions;
  });

  // Update CSS custom property when chart dimensions change
  $: if (chartContainer && $chartDimensions) {
    debugLog('D3BiWheelChart: Setting chart size CSS property:', $chartDimensions.chartSize);
    ...
    if (chartContainer) {
      resizeObserver = new ResizeObserver((entries) => {
        for (const entry of entries) {
          debugLog('D3BiWheelChart: Container resized:', entry.contentRect);
  ...
  $: if (transitData && chartContainer) {
    ...
    debugLog('D3BiWheel: Transit data reactive block triggered:', { 
    ...
    if (currentHash !== lastTransitHash && transitData.trim()) {
      ...
      debugLog('D3BiWheel: Parsing new transit data');
  ...
  $: if (chartContainer && !chartStateDataInitialized()) {
    ...
    if (storeData) {
      debugLog('D3Chart: Container now ready, parsing stored natal chart data');
  ...
  $: if (get(chartState).data.length > 0 || get(transitState).data.length > 0) {
    if (chartContainer) {
      debugLog('D3BiWheel: State change detected, redrawing chart');
      createChart();
    }
  }
  ...
  function detectDeviceType() {
    ...
    debugLog('D3Chart: Device detection - width:', width, 'layout:', layout);
      ...
      if ((planet as any).house === undefined) {
        ...
        if (target === 'transit') {
          ...
          if (natalState.houseCusps && natalState.houseCusps.length > 0) {
            ...
            debugLog(`D3Chart: Using natal house cusps for transit planet ${planet.planet}`);
          }
        }
        ...
        debugLog(`D3Chart: Calculated house for ${planet.planet}: ${houseNumber} (using ${target === 'transit' ? 'natal' : 'own'} house cusps)`);
      }
    });
  ...
  function createChart() {
    ...
    if (trans.data.length) {
      ...
      if (showAspectLines) {
        debugLog('D3BiWheel: Drawing transit-to-natal aspects');
        drawAspects(g, house1CuspAngle, false);
      }
    } else {
      debugLog('D3BiWheel: No transit data to draw');
  ...
  function drawPlanets(g: d3.Selection<SVGGElement, unknown, null, undefined>, house1CuspAngle: number, isInner: boolean) {
    ...
    if (!natalAsc) {
      debugLog('D3Chart: No natal ASC found for planet positioning');
      return;
    }
    
    const natalAscSignIndex = zodiacSigns.indexOf(natalAsc.sign as typeof zodiacSigns[number]);
