...
export function drawTransitChart(
    container: HTMLElement,
    natalChart: NatalChart,
    transitChart: TransitChart,
    options: ChartOptions = {}
) {
    const { showAspectLines = true } = options;
    ...
    const natalPlanetRingRadius = radius * 0.65;
    const transitPlanetRingRadius = radius * 0.5;
    const aspectHubRadius = radius * 0.4;
    const houseLineInnerRadius = radius * 0.4;
    const houseNumRadius = radius * 0.35;

    const ascAngle = natalChart.ascendant;

    drawZodiacWheel(svg, zodiacOuterRadius, zodiacInnerRadius, ascAngle);
    drawHouseLines(svg, natalChart.houses, zodiacInnerRadius, houseLineInnerRadius, houseNumRadius, ascAngle);
    drawPlanets(svg, natalChart.planets, natalPlanetRingRadius, ascAngle, 'natal');
    drawPlanets(svg, transitChart.planets, transitPlanetRingRadius, ascAngle, 'transit');

    if (showAspectLines) {
        const aspects = calculateTransitAspects(natalChart.planets, transitChart.planets);
...
function drawZodiacWheel(svg: d3.Selection<any, any, any, any>, outerRadius: number, innerRadius: number, ascAngle: number) {
...
function drawHouseLines(svg: d3.Selection<any, any, any, any>, houses: HouseCusp[], zodiacInnerRadius: number, houseLineInnerRadius: number, houseNumRadius: number, ascAngle: number) {
    ...
    houses.forEach((house, i) => {
        ...
        houseGroup.append('line')
            ...
            .attr('stroke', i === 0 || i === 3 || i === 6 || i === 9 ? '#888' : '#ddd')
            .attr('stroke-width', i === 0 || i === 3 || i === 6 || i === 9 ? 1.5 : 1);
        
        const nextHouse = houses[(i + 1) % 12];
        const nextAngle = nextHouse ? (nextHouse.longitude - ascAngle + 360) % 360 : angle + 30;
        const midAngle = angle + (d3.arc().startAngle(angle).endAngle(nextAngle) as any).angle / 2;
...
function drawPlanets(svg: d3.Selection<any, any, any, any>, planets: Planet[], ringRadius: number, ascAngle: number, type: 'natal' | 'transit') {
...
function calculateTransitAspects(natalPlanets: Planet[], transitPlanets: Planet[]) {
...
function drawAspects(svg: d3.Selection<any, any, any, any>, aspects: TransitAspect[], natalRadius: number, transitRadius: number, ascAngle: number) {
